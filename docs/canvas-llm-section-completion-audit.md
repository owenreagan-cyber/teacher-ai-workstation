...
4. Read `docs/canvas-llm-planning-foundation-freeze.md` for unfreeze requirements.
5. Confirm separate explicit approval for the proposed runtime/export/API/generation scope.
6. Create a named PR with explicit scope and dry-run plan if runtime is involved.
7. Do not assume Canvas is the default track — approval must be explicit.
8. Re-run `bash scripts/teacher-app-designer-canvas-llm-status.sh` and `bin/chief-of-staff --dashboard` after changes.
