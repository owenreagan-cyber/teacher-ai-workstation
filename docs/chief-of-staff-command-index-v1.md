...
| `bin/chief-of-staff --classroom-timer-stopwatch-runtime-status` | Classroom Timer & Stopwatch Level 3 runtime prototype status (does not launch app) |
| `bin/chief-of-staff --app-runtime-approval-gate-status` | App runtime approval gate (Timer only approved) |
...
| `bin/chief-of-staff --master-plan-status` | Durable master plan persistence and program roadmap consolidation proof |
...
| `bin/chief-of-staff --curriculum-production-registry-owen-checklist-status` | Owen § J production registry approval checklist tracker (planning-only; Owen decisions required) — companion: `docs/curriculum-builder-production-registry-owen-decision-worksheet.md` |
...
| `bin/chief-of-staff --canvas-llm-phase-7-status` | Canvas LLM Phase 7 read-only Canvas API approval packet (approval packet only; no API/OAuth/live reads) |
...
| `bin/chief-of-staff --canvas-llm-phase-11-status` | Canvas LLM Phase 11 sandbox metadata import preview gate status (preview only; no import or database writes) |
