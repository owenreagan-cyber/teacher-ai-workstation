...
| Canvas LLM | **DOCS-ONLY** | `docs/canvas-llm-planning-foundation-freeze.md`; `docs/canvas-llm-local-first-drive-first-architecture.md`; `docs/programs/canvas-llm/`; `fixtures/canvas-llm/` | `--teacher-app-designer-canvas-llm-status`; `--canvas-llm-phase-0-status`; `--canvas-llm-phase-1-status`; `--canvas-llm-phase-2-status`; `--canvas-llm-phase-3-status`; `--canvas-llm-phase-4-status`; `--canvas-llm-phase-5-status`; `--canvas-llm-phase-6-status`; `--canvas-llm-phase-7-status`; `--canvas-llm-phase-8-status`; `--canvas-llm-phase-9a-status`; `--canvas-llm-phase-9b-status`; `--canvas-llm-phase-10-status`; `--canvas-llm-phase-11-status` | Canvas LLM Phase 0 standards foundation, Canvas LLM Phase 1 fake/local validator, Phase 2 manual evidence intake, Phase 3 manual evidence normalizer, Canvas LLM Phase 4 knowledge architecture and pattern catalog, Canvas LLM Phase 5 fake/local knowledge DB prototype, Canvas LLM Phase 6 fake/local knowledge DB validator, Canvas LLM Phase 7 read-only API approval packet, Canvas LLM Phase 8 sandbox/demo API fetch gate, Canvas LLM Phase 9A sandbox metadata fetch scaffold, Canvas LLM Phase 9B sandbox API metadata fetch execution, Canvas LLM Phase 10 sandbox metadata review/import gate, and Canvas LLM Phase 11 sandbox metadata import preview gate are local-first guarded metadata-only systems. Runtime/export/API/OAuth/generation/deployment/student data are frozen and blocked. Supabase language in older Canvas docs is historical/optional, not default. | Keep Canvas validation limited to fixed fake/local fixtures until explicit unfreeze approval. |
...
Build-state Canvas Phase 11: SANDBOX METADATA IMPORT PREVIEW GATE COMPLETE. Evidence: `docs/programs/canvas-llm/canvas-phase-11-sandbox-import-preview-gate.md`, `scripts/canvas-llm-sandbox-import-preview.py`, `scripts/canvas-llm-phase-11-status.sh`, and `bin/chief-of-staff --canvas-llm-phase-11-status`. Canvas LLM Phase 11 maps ignored local Phase 9B metadata for course 24399 into preview-only candidate entity types, preserves the expected announcements_metadata=0 warning, and performs no Canvas API calls, metadata import, knowledge DB writes, runtime SQLite/database writes, production writes, student-data handling, real curriculum body ingestion, generation/runtime behavior, embeddings/RAG, or local model execution. Next safe step is Phase 12 fake/local sandbox metadata import artifact gate, not production course expansion.

Build-state Canvas Phase 10: SANDBOX METADATA REVIEW/IMPORT GATE COMPLETE. Evidence: `docs/programs/canvas-llm/canvas-phase-10-sandbox-metadata-review-gate.md`, `scripts/canvas-llm-sandbox-metadata-review.py`, `scripts/canvas-llm-phase-10-status.sh`, and `bin/chief-of-staff --canvas-llm-phase-10-status`. Canvas LLM Phase 10 reviews ignored local Phase 9B metadata for course 24399 only, preserves the expected announcements_metadata=0 warning, and performs no Canvas API calls, metadata import, production writes, student-data handling, real curriculum body ingestion, runtime SQLite/database writes, generation/runtime behavior, embeddings/RAG, or local model execution. Next safe step is Phase 11 sandbox metadata import preview gate, not production course expansion.

Build-state Canvas Phase 9B: FIRST APPROVED READ-ONLY SANDBOX METADATA FETCH COMPLETE. Evidence: `docs/programs/canvas-llm/canvas-phase-9b-sandbox-api-metadata-fetch.md`, `scripts/canvas-llm-sandbox-metadata-fetch.py`, `scripts/canvas-llm-phase-9b-status.sh`, and `bin/chief-of-staff --canvas-llm-phase-9b-status`. Canvas LLM Phase 9B accepted course 24399 only, used local env variables only, wrote sanitized metadata only to ignored `.local/canvas-llm/sandbox-metadata/course-24399/`, and did not commit fetched metadata. It does not authorize other courses, production course access, historical course access, Canvas writes/publishing, student data, real curriculum body ingestion, runtime SQLite/database writes, generation/runtime behavior, embeddings/RAG, local model execution, or production writes. Next safe step is Phase 10 sandbox metadata review/import gate, not production course expansion.

Build-state Canvas Phase 9A: DRY-RUN SCAFFOLD ONLY / NO LIVE FETCH IMPLEMENTED. Evidence: `docs/programs/canvas-llm/canvas-phase-9a-sandbox-metadata-fetch-scaffold.md`, `scripts/canvas-llm-sandbox-metadata-fetch-scaffold.py`, `scripts/canvas-llm-phase-9a-status.sh`, and `bin/chief-of-staff --canvas-llm-phase-9a-status`. Canvas LLM Phase 9A does not perform Canvas API/OAuth/live reads, does not print or write tokens, does not authorize Canvas writes/publishing, student data, real curriculum body ingestion, production course access, historical course access, generation/runtime behavior, embeddings/RAG, runtime SQLite/database writes, or production writes. Course 24399 is the only accepted course ID in the scaffold; live fetch remains blocked for Phase 9B or later.

Build-state Canvas Phase 8: API FETCH GATE ONLY / NO API. Evidence: `docs/programs/canvas-llm/canvas-phase-8-sandbox-api-fetch-gate.md`, `scripts/canvas-llm-phase-8-status.sh`, and `bin/chief-of-staff --canvas-llm-phase-8-status`. Canvas LLM Phase 8 does not authorize Canvas API/OAuth/live reads, access token use, Canvas writes/publishing, student data, real curriculum ingestion, production course access, historical course access, network access, generation/runtime behavior, embeddings/RAG, runtime SQLite/database writes, or production writes. Demo/sandbox course 24399 remains only a future candidate until separately approved for a read-only metadata fetch.

Build-state Canvas Phase 7: APPROVAL PACKET ONLY / NO API. Evidence: `docs/programs/canvas-llm/canvas-phase-7-read-only-api-approval-packet.md`, `scripts/canvas-llm-phase-7-status.sh`, and `bin/chief-of-staff --canvas-llm-phase-7-status`. Canvas LLM Phase 7 does not authorize Canvas API/OAuth/live reads, access tokens, Canvas writes/publishing, student data, real curriculum ingestion, production course access, historical course access, network access, generation/runtime behavior, embeddings/RAG, runtime SQLite/database writes, or production writes. Demo/sandbox course 24399 is documented only as a future candidate for a separately approved Phase 8 read-only sandbox/demo test.
