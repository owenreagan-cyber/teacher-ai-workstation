...
if [[ -f scripts/teacher-knowledge-vault-m7-connector-approval-status.sh ]]; then
  teacher_knowledge_vault_m7_result=0
  teacher_knowledge_vault_m7_output="$(bash scripts/teacher-knowledge-vault-m7-connector-approval-status.sh 2>&1)" || teacher_knowledge_vault_m7_result=$?
  teacher_knowledge_vault_m7_pass="$(summary_count "${teacher_knowledge_vault_m7_output}" "PASS")"
  teacher_knowledge_vault_m7_warn="$(summary_count "${teacher_knowledge_vault_m7_output}" "WARN")"
  teacher_knowledge_vault_m7_fail="$(summary_count "${teacher_knowledge_vault_m7_output}" "FAIL")"

  if [[ "${teacher_knowledge_vault_m7_result}" != "0" ]]; then
    printf '%s\n' "${teacher_knowledge_vault_m7_output}"
    fail "Teacher Knowledge Vault M7 connector approval status failed"
  elif [[ -n "${teacher_knowledge_vault_m7_pass}" && -n "${teacher_knowledge_vault_m7_warn}" && -n "${teacher_knowledge_vault_m7_fail}" ]]; then
    printf 'Teacher Knowledge Vault M7 Connector Approval: PASS %s / WARN %s / FAIL %s\n' "${teacher_knowledge_vault_m7_pass}" "${teacher_knowledge_vault_m7_warn}" "${teacher_knowledge_vault_m7_fail}"
    pass "Teacher Knowledge Vault M7 connector approval status completed"
  else
    pass "Teacher Knowledge Vault M7 connector approval status completed"
  fi
else
...
if [[ -f scripts/teacher-knowledge-vault-m7f-persistent-working-catalog-approval-gate-status.sh ]]; then
  teacher_knowledge_vault_m7f_result=0
  teacher_knowledge_vault_m7f_output="$(bash scripts/teacher-knowledge-vault-m7f-persistent-working-catalog-approval-gate-status.sh 2>&1)" || teacher_knowledge_vault_m7f_result=$?
  teacher_knowledge_vault_m7f_pass="$(summary_count "${teacher_knowledge_vault_m7f_output}" "PASS")"
  teacher_knowledge_vault_m7f_warn="$(summary_count "${teacher_knowledge_vault_m7f_output}" "WARN")"
  teacher_knowledge_vault_m7f_fail="$(summary_count "${teacher_knowledge_vault_m7f_output}" "FAIL")"

  if [[ "${teacher_knowledge_vault_m7f_result}" != "0" ]]; then
    printf '%s\n' "${teacher_knowledge_vault_m7f_output}"
    fail "Teacher Knowledge Vault M7f persistent working catalog approval gate status failed"
  elif [[ -n "${teacher_knowledge_vault_m7f_pass}" && -n "${teacher_knowledge_vault_m7f_warn}" && -n "${teacher_knowledge_vault_m7f_fail}" ]]; then
    printf 'Teacher Knowledge Vault M7f Persistent Working Catalog Approval Gate: PASS %s / WARN %s / FAIL %s\n' "${teacher_knowledge_vault_m7f_pass}" "${teacher_knowledge_vault_m7f_warn}" "${teacher_knowledge_vault_m7f_fail}"
    pass "Teacher Knowledge Vault M7f persistent working catalog approval gate status completed"
  else
    pass "Teacher Knowledge Vault M7f persistent working catalog approval gate status completed"
  fi
else
...
if [[ -f scripts/teacher-knowledge-vault-m2d-selected-folder-status.sh ]]; then
  teacher_knowledge_vault_m2d_result=0
  teacher_knowledge_vault_m2d_output="$(bash scripts/teacher-knowledge-vault-m2d-selected-folder-status.sh 2>&1)" || teacher_knowledge_vault_m2d_result=$?
  teacher_knowledge_vault_m2d_pass="$(summary_count "${teacher_knowledge_vault_m2d_output}" "PASS")"
  teacher_knowledge_vault_m2d_warn="$(summary_count "${teacher_knowledge_vault_m2d_output}" "WARN")"
  teacher_knowledge_vault_m2d_fail="$(summary_count "${teacher_knowledge_vault_m2d_output}" "FAIL")"

  if [[ "${teacher_knowledge_vault_m2d_result}" != "0" ]]; then
    printf '%s\n' "${teacher_knowledge_vault_m2d_output}"
    fail "Teacher Knowledge Vault M2d first selected folder metadata scan status failed"
  elif [[ -n "${teacher_knowledge_vault_m2d_pass}" && -n "${teacher_knowledge_vault_m2d_warn}" && -n "${teacher_knowledge_vault_m2d_fail}" ]]; then
    printf 'Teacher Knowledge Vault M2d First Selected Folder Metadata Scan: PASS %s / WARN %s / FAIL %s\n' "${teacher_knowledge_vault_m2d_pass}" "${teacher_knowledge_vault_m2d_warn}" "${teacher_knowledge_vault_m2d_fail}"
    pass "Teacher Knowledge Vault M2d first selected folder metadata scan status completed"
  else
    pass "Teacher Knowledge Vault M2d first selected folder metadata scan status completed"
  fi
else
  warn "Teacher Knowledge Vault M2d first selected folder metadata scan status script missing: scripts/teacher-knowledge-vault-m2d-selected-folder-status.sh"
fi
