#!/usr/bin/env bash
# Read-only Curriculum Library v1 foundation status only. No scanning, network calls, or writes.
set -euo pipefail
...
cat <<'EOF'
Status: documentation/status/reference model only
Real curriculum records: no
File duplication: no
Folder scanning: no
Indexing: no
APIs: no
