#!/usr/bin/env bash
# Read-only Document Indexing command detail status only. No network calls or file creation.
set -euo pipefail
...
detail_doc="docs/document-indexing-command-detail-polish.md"
followup_doc="docs/local-document-indexing-follow-up.md"
...
cat <<'EOF'
Status: document-indexing-command-detail/status only
...
Document Indexing commands documented: yes
...
if [[ -f "${detail_doc}" ]]; then
  check_doc_contains "${detail_doc}" "Document Indexing Command Detail Polish" "Document Indexing Command Detail Polish"
  check_doc_contains "${detail_doc}" "## Document Indexing Planning Commands" "Document Indexing Planning Commands"
  ...
  check_doc_contains "${detail_doc}" "Teacher workflow command detail summary" "next recommended PR Teacher workflow command detail summary"
  ...
  check_doc_contains "${detail_doc}" "no folder scanning" "no folder scanning"
  check_doc_contains "${detail_doc}" "no file indexing" "no file indexing"
  check_doc_contains "${detail_doc}" "no content parsing" "no content parsing"
  check_doc_contains "${detail_doc}" "no OCR" "no OCR"
  check_doc_contains "${detail_doc}" "no embeddings" "no embeddings"
  check_doc_contains "${detail_doc}" "no vector database" "no vector database"
