#!/usr/bin/env bash
# Read-only Lesson Review command detail status only. No network calls or file creation.
set -euo pipefail
...
detail_doc="docs/lesson-review-command-detail-polish.md"
...
cat <<'EOF'
Status: lesson-review-command-detail/status only
...
Lesson Review command documented: yes
...
if [[ -f "${detail_doc}" ]]; then
  check_doc_contains "${detail_doc}" "Lesson Review Command Detail Polish" "Lesson Review Command Detail Polish"
  check_doc_contains "${detail_doc}" "## Lesson Review Command" "Lesson Review Command"
  ...
  check_doc_contains "${detail_doc}" "Review notes command detail polish" "next recommended PR Review notes command detail polish"
