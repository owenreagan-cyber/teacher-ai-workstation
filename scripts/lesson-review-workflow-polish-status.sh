#!/usr/bin/env bash
# Read-only lesson review workflow polish status only. No network calls or file creation.
set -euo pipefail
...
polish_doc="docs/lesson-review-workflow-polish.md"
...
if [[ -f "${polish_doc}" ]]; then
  check_doc_contains "lesson review workflow polish" "lesson review workflow polish"
  ...
  check_doc_contains "Review Notes Guidance" "review notes guidance"
