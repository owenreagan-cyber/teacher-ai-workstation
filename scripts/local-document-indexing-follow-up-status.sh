#!/usr/bin/env bash
# Read-only local document indexing follow-up status only. No network calls or file creation.
set -euo pipefail
...
check_doc_contains() {
  local phrase="$1"
  local label="$2"
  if grep -Fq "${phrase}" "${followup_doc}"; then
...
if [[ -f "${followup_doc}" ]]; then
  check_doc_contains "local document indexing follow-up" "local document indexing follow-up"
  ...
  check_doc_contains "no folder scanning" "no folder scanning"
  check_doc_contains "no file indexing" "no file indexing"
  check_doc_contains "no file content parsing" "no file content parsing"
