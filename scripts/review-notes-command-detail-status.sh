#!/usr/bin/env bash
# Read-only Review Notes command detail status only. No network calls or file creation.
set -euo pipefail
...
detail_doc="docs/review-notes-command-detail-polish.md"
...
cat <<'EOF'
Status: review-notes-command-detail/status only
...
Review Notes command documented: yes
...
if [[ -f "${detail_doc}" ]]; then
  check_doc_contains "${detail_doc}" "Review Notes Command Detail Polish" "Review Notes Command Detail Polish"
  check_doc_contains "${detail_doc}" "## Review Notes Command" "Review Notes Command"
  ...
  check_doc_contains "${detail_doc}" "Document indexing command detail polish" "next recommended PR Document indexing command detail polish"
