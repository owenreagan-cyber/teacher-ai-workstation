...
for f in scripts/*knowledge-vault* assistant/teacher-knowledge-vault/m1/*; do
  ...
  if grep -qiE '(sqlite3|\.db"|CREATE TABLE.*EXECUTE)' "${f}" 2>/dev/null; then
    if [[ "${f}" != *catalog-schema-direction* ]] && [[ "${f}" != *.md ]] && [[ "${f}" != *m7e-local-test-catalog-* ]] && [[ "${f}" != *m7g-persistent-working-catalog-* ]] && [[ "${f}" != *m2b-repo-staging-metadata-* ]]; then
      sqlite_hits=1
...
check_smoke_excludes_deep_validation 'teacher-knowledge-vault-m1' 'Teacher Knowledge Vault M1'
