#!/usr/bin/env bash
# Teacher Knowledge Vault M7e local test catalog import — fixed fixture paths only.
set -euo pipefail
...
M7E_DB="${M7E_OUT_DIR}/test-catalog.sqlite"
M7E_BACKUP="${M7E_OUT_DIR}/test-catalog-backup.sqlite"
M7E_SUMMARY="${M7E_OUT_DIR}/import-summary.json"
M7E_ROLLBACK="${M7E_OUT_DIR}/rollback-proof.json"
...
section 'Local Test Catalog Import'
import_output="$(python3 - "${repo_root}" "${M7E_OUT_DIR}" "${M7E_DB}" "${M7E_BACKUP}" "${M7E_SUMMARY}" "${M7E_ROLLBACK}" "${M7B_INVENTORY}" "${M7C_PREVIEW}" <<'PY'
import json, os, shutil, sqlite3, sys

repo_root, out_dir, db_path, backup_path, summary_path, rollback_path, inventory_path, preview_path = sys.argv[1:9]
batch_id = "fake-m7e-batch-001"
...
conn = sqlite3.connect(os.path.join(repo_root, db_path))
cur = conn.cursor()
cur.executescript("""
CREATE TABLE import_batches (
  ...
  fixture_source TEXT,
  created_at TEXT,
  runtime_test_catalog_only INTEGER,
...
cur.execute(
    ...
    (batch_id, "m7b+m7c fixtures", created_at, 1, 0),
)
counts["import_batches"] += 1

for ev in [
    ("fake-m7e-evt-import-start", "test_catalog_import_started", None),
    ("fake-m7e-evt-validator", "m7c_fixture_validator_attached", None),
    ("fake-m7e-evt-preview", "import_preview_candidates_imported", None),
    ("fake-m7e-evt-import-done", "test_catalog_import_completed", None),
]:
    cur.execute(
...
summary = {
    ...
    "runtime_test_catalog_only": True,
...
rollback = {
    "classification": "fake_local_planning_only",
    "runtime_test_catalog_only": True,
    "rollback_plan_id": "fake-m7e-rollback-001",
    "import_batch_id": batch_id,
...
[[ -f "${M7E_DB}" ]] && pass "test catalog sqlite exists at fixed path" || fail "test catalog sqlite missing"
[[ -f "${M7E_SUMMARY}" ]] && pass "import summary written" || fail "import summary missing"
[[ -f "${M7E_ROLLBACK}" ]] && pass "rollback proof written" || fail "rollback proof missing"
grep -Fq -- '"production_write": false' "${M7E_SUMMARY}" && pass 'summary shows production_write false' || fail 'summary must show production_write false'
