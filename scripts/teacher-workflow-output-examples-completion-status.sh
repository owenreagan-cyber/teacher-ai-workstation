#!/usr/bin/env bash
# Read-only Teacher Workflow output examples completion marker status only. No network calls or file creation.
set -euo pipefail
...
cat <<'EOF'
Status: teacher-workflow-output-examples-completion/status only
...
Command detail stack complete for now: yes
Safe-output example stack complete for now: yes
...
section 'Completion Marker Doc Checks'

check_file "${completion_doc}"

if [[ -f "${completion_doc}" ]]; then
  check_doc_contains "${completion_doc}" "Teacher Workflow Output Examples Completion Marker" "Teacher Workflow Output Examples Completion Marker"
  check_doc_contains "${completion_doc}" "Teacher Workflow command detail and safe-output example stack is complete for now" "stack complete for now"
  check_doc_contains "${completion_doc}" "safe return point" "safe return point"
  check_doc_contains "${completion_doc}" "Lesson-planning template readiness polish" "Lesson-planning template readiness polish"
  check_doc_contains "${completion_doc}" "Completed Command Detail Stack" "Completed Command Detail Stack"
  check_doc_contains "${completion_doc}" "Completed Safe-Output Stack" "Completed Safe-Output Stack"
  check_doc_contains "${completion_doc}" "Verification Commands" "Verification Commands"
  check_doc_contains "${completion_doc}" "Lesson-Planning Template Return Rules" "Lesson-Planning Template Return Rules"
  check_doc_contains "${completion_doc}" "Future Teacher Workflow Output Work" "Future Teacher Workflow Output Work"
  ...
  check_doc_contains "${completion_doc}" "no folder scanning" "no folder scanning"
  check_doc_contains "${completion_doc}" "no file indexing" "no file indexing"
  check_doc_contains "${completion_doc}" "no content parsing" "no content parsing"
  check_doc_contains "${completion_doc}" "no OCR" "no OCR"
  check_doc_contains "${completion_doc}" "no embeddings" "no embeddings"
  check_doc_contains "${completion_doc}" "no vector database" "no vector database"
