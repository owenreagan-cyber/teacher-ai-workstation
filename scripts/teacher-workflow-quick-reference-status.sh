#!/usr/bin/env bash
# Read-only Teacher Workflow quick-reference status only. No network calls or file creation.
set -euo pipefail
...
cat <<'EOF'
Status: teacher-workflow-quick-reference/status only
...
Safe teacher commands documented: yes
Teacher workflow docs documented: yes
Future-only work labeled: yes
...
if [[ -f "${quick_ref_doc}" ]]; then
  check_doc_contains "${quick_ref_doc}" "Teacher Workflow Quick-Reference Polish" "Teacher Workflow Quick-Reference Polish"
  check_doc_contains "${quick_ref_doc}" "Safe Teacher Workflow Commands" "safe teacher workflow commands"
  check_doc_contains "${quick_ref_doc}" "Teacher Planning References" "teacher planning references"
  check_doc_contains "${quick_ref_doc}" "Lesson Review References" "lesson review references"
  check_doc_contains "${quick_ref_doc}" "Review Notes References" "review notes references"
  check_doc_contains "${quick_ref_doc}" "Document Indexing Planning References" "document indexing planning references"
  check_doc_contains "${quick_ref_doc}" "Parked and Future-Only Work" "parked and future-only work"
  check_doc_contains "${quick_ref_doc}" "preserving all existing commands" "preserving all existing commands"
  check_doc_contains "${quick_ref_doc}" "no command removals" "no command removals"
  check_doc_contains "${quick_ref_doc}" "no command renames" "no command renames"
  check_doc_contains "${quick_ref_doc}" "no check removals" "no check removals"
  check_doc_contains "${quick_ref_doc}" "preserving command behavior" "preserving command behavior"
  check_doc_contains "${quick_ref_doc}" "PASS/WARN/FAIL semantics" "preserving PASS/WARN/FAIL semantics"
  check_doc_contains "${quick_ref_doc}" "no document scanning" "no document scanning"
  check_doc_contains "${quick_ref_doc}" "no file indexing" "no file indexing"
  check_doc_contains "${quick_ref_doc}" "no lesson generation changes" "no lesson generation changes"
  check_doc_contains "${quick_ref_doc}" "no real review notes" "no real review notes"
  check_doc_contains "${quick_ref_doc}" "no student data" "no student data"
  check_doc_contains "${quick_ref_doc}" "no network calls" "no network calls"
  check_doc_contains "${quick_ref_doc}" "no automation" "no automation"
  check_doc_contains "${quick_ref_doc}" "Live wallpaper/photo curator implementation is not started" "live wallpaper/photo curator implementation is not started"
  check_doc_contains "${quick_ref_doc}" "3D Design Factory Agent remains parked" "3D Design Factory Agent remains parked"
fi
