#!/usr/bin/env bash
# Read-only Teacher Workflow safe-output examples status only. No network calls or file creation.
set -euo pipefail
...
cat <<'EOF'
Status: teacher-workflow-safe-output-examples/status only
...
Safe output examples added: yes
Planning-only labels visible: yes
Placeholder counts documented: yes
...
if [[ -f "${examples_doc}" ]]; then
  check_doc_contains "${examples_doc}" "Teacher Workflow Safe-Output Examples" "Teacher Workflow Safe-Output Examples"
  check_doc_contains "${examples_doc}" "Safe Output Example Rules" "Safe Output Example Rules"
  check_doc_contains "${examples_doc}" "## Teacher Planning Safe Output" "Teacher Planning Safe Output"
  check_doc_contains "${examples_doc}" "## Lesson Review Safe Output" "Lesson Review Safe Output"
  check_doc_contains "${examples_doc}" "## Review Notes Safe Output" "Review Notes Safe Output"
  check_doc_contains "${examples_doc}" "## Document Indexing Planning Safe Output" "Document Indexing Planning Safe Output"
  check_doc_contains "${examples_doc}" "Safe Output Must Show" "Safe Output Must Show"
  check_doc_contains "${examples_doc}" "Safe Output Must Not Show" "Safe Output Must Not Show"
  check_doc_contains "${examples_doc}" "Related Command Detail Docs" "Related Command Detail Docs"
  check_doc_contains "${examples_doc}" "Future Prompt Use Rules" "Future Prompt Use Rules"
  check_doc_contains "${examples_doc}" "Teacher workflow safe-output checker" "next recommended PR Teacher workflow safe-output checker"
  check_doc_contains "${examples_doc}" "preserving all existing commands" "preserving all existing commands"
  check_doc_contains "${examples_doc}" "no command removals" "no command removals"
  check_doc_contains "${examples_doc}" "no command renames" "no command renames"
  check_doc_contains "${examples_doc}" "no check removals" "no check removals"
  check_doc_contains "${examples_doc}" "preserving command behavior" "preserving command behavior"
  check_doc_contains "${examples_doc}" "PASS/WARN/FAIL semantics" "preserving PASS/WARN/FAIL semantics"
  check_doc_contains "${examples_doc}" "placeholders in this documentation example" "X, Y, and Z are placeholders"
  check_doc_contains "${examples_doc}" "no document scanning" "no document scanning"
  check_doc_contains "${examples_doc}" "no folder scanning" "no folder scanning"
  check_doc_contains "${examples_doc}" "no file indexing" "no file indexing"
  check_doc_contains "${examples_doc}" "no content parsing" "no content parsing"
  check_doc_contains "${examples_doc}" "no OCR" "no OCR"
  check_doc_contains "${examples_doc}" "no embeddings" "no embeddings"
  check_doc_contains "${examples_doc}" "no vector database" "no vector database"
  check_doc_contains "${examples_doc}" "no lesson generation changes" "no lesson generation changes"
  check_doc_contains "${examples_doc}" "no real review notes" "no real review notes"
  check_doc_contains "${examples_doc}" "no student data" "no student data"
  check_doc_contains "${examples_doc}" "no network calls" "no network calls"
  check_doc_contains "${examples_doc}" "no automation" "no automation"
fi
