#!/usr/bin/env bash
# Read-only Teacher Workflow status summary only. No network calls or file creation.
set -euo pipefail
...
cat <<'EOF'
Status: teacher-workflow-status-summary/status only
...
Teacher workflow stack summarized: yes
Planning-only areas labeled: yes
Future-only areas labeled: yes
...
if [[ -f "${summary_doc}" ]]; then
  check_doc_contains "${summary_doc}" "Teacher Workflow Status Summary" "Teacher Workflow Status Summary"
  check_doc_contains "${summary_doc}" "Teacher Workflow Stack Status" "teacher workflow stack status"
  check_doc_contains "${summary_doc}" "Complete For Now" "complete for now"
  check_doc_contains "${summary_doc}" "Safe Status and Planning Commands" "safe status and planning commands"
  check_doc_contains "${summary_doc}" "Planning-Only Areas" "planning-only areas"
  check_doc_contains "${summary_doc}" "Future-Only Areas" "future-only areas"
  check_doc_contains "${summary_doc}" "Not Implemented" "not implemented"
  check_doc_contains "${summary_doc}" "Dashboard Status Summary" "dashboard status summary"
  check_doc_contains "${summary_doc}" "Teacher planning command detail polish" "next recommended PR Teacher planning command detail polish"
  ...
  check_doc_contains "${summary_doc}" "Wallpaper/photo curator live behavior is not implemented" "live wallpaper/photo curator live behavior is not implemented"
  check_doc_contains "${summary_doc}" "3D Design Factory Agent behavior is not implemented" "3D Design Factory Agent behavior is not implemented"
fi
