...
for closure in \
  ...
  complete_teacher_knowledge_vault_m2_local_discovery_approval_packet \
  ...
  complete_teacher_knowledge_vault_m7_read_only_connector_approval_packet \
  complete_teacher_knowledge_vault_m7b_manual_source_inventory_level_1 \
  complete_teacher_knowledge_vault_m7c_manual_inventory_import_preview \
  ...
  complete_teacher_knowledge_vault_m2b_repo_owned_staging_metadata_prototype \
  complete_teacher_knowledge_vault_m2c_selected_local_folder_approval_gate \
  complete_teacher_knowledge_vault_m2d_first_selected_folder_metadata_scan; do
...
check_doc_contains "${whole_system_report}" "159 / 0 / 0 PASS" "dashboard count current"
check_doc_contains "${whole_system_report}" "78 / 0 / 0 PASS" "validate-all count current"
if grep -Fq -- 'Dashboard 158/0/0' "${whole_system_report}" 2>/dev/null; then
  fail 'whole-system report must not contain stale Dashboard 158/0/0 example'
else
  pass 'whole-system report excludes stale Dashboard 158/0/0'
fi
if grep -Fq -- 'Validate-all 77/0/0' "${whole_system_report}" 2>/dev/null; then
  fail 'whole-system report must not contain stale Validate-all 77/0/0 example'
else
  pass 'whole-system report excludes stale Validate-all 77/0/0'
fi
if grep -Fq -- 'Dashboard 157/0/0' "${whole_system_report}" 2>/dev/null; then
  fail 'whole-system report must not contain stale Dashboard 157/0/0 example'
else
  pass 'whole-system report excludes stale Dashboard 157/0/0'
fi
if grep -Fq -- 'Validate-all 76/0/0' "${whole_system_report}" 2>/dev/null; then
  fail 'whole-system report must not contain stale Validate-all 76/0/0 example'
else
  pass 'whole-system report excludes stale Validate-all 76/0/0'
fi
...
if grep -Fq -- 'Dashboard 155/0/0' "${whole_system_report}" 2>/dev/null; then
  ...
  pass 'whole-system report excludes stale Dashboard 155/0/0'
fi
if grep -Fq -- 'Validate-all 74/0/0' "${whole_system_report}" 2>/dev/null; then
  fail 'whole-system report must not contain stale Validate-all 74/0/0 example'
else
  pass 'whole-system report excludes stale Validate-all 74/0/0'
fi
if grep -Fq -- 'Dashboard 154/0/0' "${whole_system_report}" 2>/dev/null; then
  fail 'whole-system report must not contain stale Dashboard 154/0/0 example'
else
  pass 'whole-system report excludes stale Dashboard 154/0/0'
fi
if grep -Fq -- 'Validate-all 73/0/0' "${whole_system_report}" 2>/dev/null; then
  fail 'whole-system report must not contain stale Validate-all 73/0/0 example'
else
  pass 'whole-system report excludes stale Validate-all 73/0/0'
fi
if grep -Fq -- 'Dashboard 143/0/0' "${whole_system_report}" 2>/dev/null; then
  ...
  pass 'whole-system report excludes stale Dashboard 143/0/0'
fi
...
if grep -Fq -- 'dashboard (138/0/0)' "${whole_system_report}" 2>/dev/null; then
  ...
  pass 'whole-system report excludes stale dashboard (138/0/0) lane text'
fi
if grep -Fq -- 'Dashboard 138/0/0' "${whole_system_report}" 2>/dev/null; then
  fail 'whole-system report must not contain stale Dashboard 138/0/0 example'
else
  pass 'whole-system report excludes stale Dashboard 138/0/0 example'
fi
if grep -Fq -- 'dashboard (128/0/0)' "${whole_system_report}" 2>/dev/null; then
