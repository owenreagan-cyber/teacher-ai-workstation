...
grep -q 'Planning-only: yes' "${tmp}" || { echo "FAIL: missing 3d planning-only banner"; cat "${tmp}"; rm -f "${tmp}"; exit 1; }
grep -q 'does not shell-invoke mesh/export/slicer tooling' "${tmp}" || { echo "FAIL: missing negative mesh/export assertion"; cat "${tmp}"; rm -f "${tmp}"; exit 1; }
rm -f "${tmp}"
